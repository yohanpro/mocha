...
```
/admin/dashboard/feed        목록 + 삭제
/admin/dashboard/feed/new    등록 폼
        │ savePost (insert)
        ▼
     posts 테이블  ──(revalidate)──▶  /feed (소비자), /admin/dashboard/feed (관리자)
```
...
4. `insert` → `revalidatePath('/admin/dashboard/feed')` + `revalidatePath('/feed')` → `redirect('/admin/dashboard/feed')`
...
- **관리자 네비게이션 없음**: `/admin/dashboard/feed`는 URL 직접 진입만 가능(대시보드 섹션 이동 메뉴 미구현). <br>NOTE(2026-07-12): 이후 `AdminNav` 사이드바가 추가되어 이 한계는 해소됨 — 문서 원문은 작성 시점 기록.
